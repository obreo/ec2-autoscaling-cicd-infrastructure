...
resource "aws_codebuild_project" "codebuild-staging" {
  count         = var.staging_resource_count == true && var.Docker_Application == false ? 1 : 0
...
#############################################################################################################################
resource "aws_codebuild_project" "codebuild_docker_staging" {
  count         = var.staging_resource_count == true && var.Docker_Application == true ? 1 : 0
  name          = "${var.name}-staging"
  description   = "This is a codebuild appliaction that is used for ${var.name}"
  build_timeout = 10 #min
  service_role  = aws_iam_role.codebuild-role.arn
