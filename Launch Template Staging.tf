...
resource "aws_launch_template" "template_staging" {
  ...
  network_interfaces {
    ...
    security_groups             = [aws_security_group.allow_access_staging[0].id]
  }
  ...
  user_data = filebase64("${local.folder_path}/userdata-staging.sh")
